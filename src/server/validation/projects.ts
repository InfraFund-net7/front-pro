...
const inviteMemberSchema = z.object({
  ...
  role: z.enum(
    invitableRoles,
    `role must be one of ${invitableRoles.join(', ')}`
  ),
});

export async function parseInviteMemberRequest(
  request: Request
): Promise<InviteMemberInput> {
  const body = await parseRequestBody(request, inviteMemberSchema);
