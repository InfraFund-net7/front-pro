...
I had a working openfort setup using my (this) account and deployed it on vercel with domain openfort.infrafund.net - everything was fine.

Then I moved the whole vercel project to Infrafund's company vercel account and just replaced the openfort public and private keys in the vercel env vars settings - which are set to cover preview and production deployment (so they are identical).
...
While the "login with google" works for dash-dev, it fails right away for `dashboard` with
